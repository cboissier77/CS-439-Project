...
class ShapeCNN_2 (nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.fc = nn.Linear(16*32*32 , num_classes)  # Fully connected layer
...
def subset_to_tensor(loader):
    """    Converts a subset of data from a DataLoader into a single tensor of images and labels.
    Args:
        loader (DataLoader): DataLoader providing batches of data.
    Returns:
        tuple: A tuple containing:
            - all_images (torch.Tensor): Concatenated tensor of images.
            - all_labels (torch.Tensor): Concatenated tensor of labels.
    """
...
def compute_confusion_matrix(model, loader, num_classes):
    """
    Computes the confusion matrix for a trained model on a given dataset.
    ...
    Returns:
        np.ndarray: A (num_classes x num_classes) confusion matrix.
    """
    model.eval()
    all_preds = []
    all_labels = []
...
def plot_confusion_matrix(cm, class_names , ch):
